                     ...
                     _In_ int       nCmdShow)
{
    ...
    pGameInstance->Add_Timer(L"Timer_Default");
    pGameInstance->Add_Timer(L"Timer_Frame");

    _float TimeAcc = {};

    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_DIRECT11FRAMEWORK));
    MSG msg = {};
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        {
            TimeAcc += pGameInstance->Get_TimeDelta(L"Timer_Default");

            if (TimeAcc >= 1.f / 60)
            {
                game->Run(pGameInstance->Get_TimeDelta(L"Timer_Frame"));
                TimeAcc = 0.0f;
            }
        }
    }
    ...
    if (dwRefCnt = Engine::Safe_Release(game))
    {
