...
HRESULT CActionAbleUI::Initialize(void* pArg)
{
    ...
    m_pTransformCom->SetScale(pActionDesc->vScale);
...
void CActionAbleUI::Update(_float fDeletaTime)
{
    ...
    // Y : 1 ~ -1 -> 0 ~ 1 로 변경
    m_pTransformCom->SetPosition({ m_vFontPos.x * g_iHalfWinSizeX + m_pTransformCom->GetScale().x * 0.5f,
                                   m_vFontPos.y * g_iHalfWinSizeY + m_pOwner->GetTransform()->GetScale().y * 0.5f, 0.f });
