...
void CEffectPartObject::Update(_float fDeletaTime)
{
    ...
    if (fabsf(m_fDistotionAccTime.x) >= 1.f)
    {
        if (m_EffectData.bIsLoop)
    ...
    if (fabsf(m_fDistotionAccTime.y) >= 1.f)
    {
        if (m_EffectData.bIsLoop)
...
HRESULT CEffectPartObject::Render()
{
    if (EFFECT_TYPE::MESH == m_EffectData.eType)
    {
        ...
        for (_uInt i = 0; i < iNumMeshes; ++i)
        {
            ...
            m_pShaderCom->Update_Shader(ENUM_CLASS(m_EffectData.eBlend_Mode));
            pModel->Render(i);
        }
    }
...
HRESULT CEffectPartObject::Apply_ConstantShaderResources()
{
    ...
    if (nullptr != m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::DIFFUSE)]->GetTexture(0))
        m_pShaderCom->Bind_SRV("g_DiffuseTexture", m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::DIFFUSE)]->GetTexture(0));

    if (nullptr != m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::NORMAL)]->GetTexture(0))
        m_pShaderCom->Bind_SRV("g_NormalTexture", m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::DIFFUSE)]->GetTexture(0));

    if (nullptr != m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::NOISE)]->GetTexture(0))
        m_pShaderCom->Bind_SRV("g_NoiseTexture", m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::DIFFUSE)]->GetTexture(0));

    if (nullptr != m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::MASK)]->GetTexture(0))
        m_pShaderCom->Bind_SRV("g_MaskTexture", m_pTextures[ENUM_CLASS(EFFECT_TEXTURE_TYPE::DIFFUSE)]->GetTexture(0));
